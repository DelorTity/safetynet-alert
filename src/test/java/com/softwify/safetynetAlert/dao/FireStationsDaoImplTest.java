...
public class FireStationsDaoImplTest {
    ...
    FireStationsDaoImpl fireStationDao = new FireStationsDaoImpl(dataStoreManager);
    ...
    public void getFireStationByAddressShouldReturnFalseWhenFireStationNotExist() {
        ...
        Optional<FireStation> fireStationByAddress = fireStationDao.findByAddress("tokyo dr");
        assertFalse(fireStationByAddress.isPresent());

        verify(dataStoreManager, times(1)).getFireStation();
    }

    @Test
    public void getFireStationByAddressShouldReturnTrueWhenFireStationExist() {
        List<FireStation> fireStations = Arrays.asList(
                FireStation.builder().address("12 tokyo dr").station(3).build(),
                FireStation.builder().address("432 bombe sr").station(0).build()
        );
